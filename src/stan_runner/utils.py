...
def find_model_in_cache(model_cache: Path, model_name: str, model_hash: int|str) -> Path:
    best_model_filename = None
    if isinstance(model_hash, int):
        model_id = base64.b64encode(abs(model_hash).to_bytes(32, byteorder="big")).decode("utf-8")
    else:
        model_id = model_hash
    for hash_char_count in range(0, len(model_id)):
        if hash_char_count == 0:
            ...
            model_filename = model_cache / f"{model_name} {model_id[:hash_char_count]}.stan"
        if model_filename.exists():
            # load the model and check its hash if it matches
            with model_filename.open("r") as f:
                model_code = f.read()
            if base64.b64encode(abs(calc_hash(model_code)).to_bytes(32, byteorder="big")).decode("utf-8") == model_id:

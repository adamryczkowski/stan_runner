...
from .nats_utils import name_topic_datadef, name_topic_modeldef, name_topic_run, STREAM_NAME
...
class MessageBroker:
    ...
    def __init__(self, server_url: str, user: str, password: str):
        self._server_raw = connect_to_nats(nats_connection=server_url, user=user, password=password)
        self._server_context = self._server_raw.jetstream()

        self._workers = {}
        self._tasks = {}
        self._task_queue = []
        self._broker_id = "broker_" + str(uuid.uuid4())[0:8]
    ...
    async def keep_alive(self):
        while True:
            last_message: RawStreamMsg = await self._server_context.get_last_msg(stream_name=STREAM_NAME,
                                                                                 subject="stan.broker.alive")
            if last_message is not None:
                if last_message.headers["broker_id"] != self._broker_id:
                    raise Exception(f"Broker id mismatch: {last_message.headers['broker_id']} != {self._broker_id}")

                time_to_last_message = time.time() - last_message.headers["timestamp"]
            else:
                time_to_last_message = float("inf")

            if (time_to_wait := min(0, WORKER_TIMEOUT_SECONDS - time_to_last_message)) > 0:
                await asyncio.sleep(time_to_wait)

            await self._server_context.publish("stan.broker.alive", b"1",
                                               headers={"broker_id": self._broker_id, "timestamp": time.time()})
            if last_message is not None:
    ...
    def the_loop(self):
        ...
        loop.create_task(self._server_raw.subscribe("stan.worker_adv.>", cb=self.handle_worker_adv))
        loop.create_task(self._server_raw.subscribe("stan.taskdef.>", cb=self.handle_task))
